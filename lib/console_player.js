...
for(var i = 0; i < matchCount; i++){
  ...
  while(!game.winner()){
    ...
    console.log("\n\n");
    game.print();
    console.log("Turn: "+game.turn().name+" ("+game.turn().symbol+")");
    console.log("Rolls:", rolls);

    var p = game.getBoard().map((x)=>{if(x.team() === game.turn().name){return x.length();}else{return 0;}}).reduce((a,b)=>{return a+b;}, 0) + game.getBars()[game.turn().name].length();
    if(p !== 15){
    ...
    while(moves.length > 0){
      var move = ais[game.turn().name](game);
